...
var logger = require("../common/log4js")("trace");
...
module.exports = function (req, res, next) {
  ...
  function traceData() {
    ...
    this.spinfocode = res.locals.spinfocode || '';
    this.tracename = '';
    this.user = res.locals.user || {};
    this.eventname = q.name || '';
    this.eventparams = q.param || '';
    this.explain = q.msg || '';
    /**
     * 网络类型
     * 返回值 QQ返回值   类型
     ...
     * ?    none NONE
     */
    this.networktype = q.network || '';
    this.url = url.parse(req.originalUrl).pathname + '?' + querystring.stringify(q);
    this.referer = req.headers['referer'] || q.referer || '';
    this.remark = '';
    this.format = function () {
      ...
      return util.format("%s %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",
        ...
        this.user.uid || '', this.user.phone || '', this.user.openid || '',
